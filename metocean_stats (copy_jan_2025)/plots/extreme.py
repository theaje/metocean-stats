...
def taylor_diagram(df,var_ref,var_comp,norm_std=True):
    import matplotlib.pyplot as plt
    import math
    ...
    var_ref: string with the name of the timeseries of reference
    ...
    ccf[0]=np.corrcoef(df[var_ref].to_numpy(),df[var_ref].to_numpy())[0,1] # Should be 1
    for i in range(len(var_comp)):
        ccf[i+1]=np.corrcoef(df[var_ref].to_numpy(),df[var_comp[i]].to_numpy())[0,1]
    # Normalized standard deviations (so that the std_dev of the reference is 1)
    ...
    if norm_std==True:
        std=std/std[0]
    # Centered pattern RMS difference (E'2 from in Taylor (2001))
    rmsdif=np.sqrt(std**2+std[0]**2-2*std*std[0]*ccf)
    max_std=int(np.max(std))+1
    if norm_std==True:
        min_std=0
    else:
        min_std=int(np.min(std))-1
        if min_std<0:
            min_std=0
    # Coordinates
    x=ccf*std
    ...
    xbc1=np.arange(0.0,max_std+0.015,0.01)
    ...
    for i in range(len(corr1)):
        ...
        #angle[i]=math.atan(yt[i]/xt[i])*180.0/math.pi
    angle=0.0-angle[::-1]
    ...
    combinations = combinations[0:len(var_comp)]
    # Plot the d ata
    ...
    levels_crms=np.arange(step,np.nanmax(zz)-step,step)
    print(np.nanmax(zz))
    print(levels_crms)
    ...
    for r in range(1,len(var_comp)+1): # Actual plotting of the data
        ax.plot(x[r],y[r],clip_on=False,marker=combinations[r-1][0],color=combinations[r-1][1],markersize=14,markerfacecolor=combinations[r-1][1])
    ...
    for i in range(len(corr1)):
        #ax.plot(xt[i],yt[i],clip_on=False,marker='o',color='pink',markersize=6,markerfacecolor='pink')
    ...
    leg=[mlines.Line2D([], [], color='k', marker='o', linestyle='None',markersize=14, label=var_ref)]
    for i in range(1,len(var_comp)+1):
        leg.append(mlines.Line2D([], [], color=combinations[i-1][1], marker=combinations[i-1][0], linestyle='None',markersize=14, label=var_comp[i-1]))

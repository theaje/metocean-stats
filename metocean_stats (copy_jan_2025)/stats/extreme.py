...
def joint_distribution_Hs_Tp(data,var_hs='hs',var_tp='tp',periods=[1,10,100,10000], adjustment=None):  
    ...
    h1 = np.linspace(start=0.01, stop=np.ceil(max(df[var_hs].values)*np.sqrt(np.pi)), num=1500)
    ...
    if 0 < mu < 5 : 
      #Based on Moan et al. (2005), "Uncertainty of wave-induced response of marine structures due to long-term variation of extratropical wave conditions":
    	pdf_Hs1 = 1/(np.sqrt(2*np.pi)*sigma*h1)*np.exp(-(np.log(h1)-alpha)**2/(2*sigma**2))
    else:
        param = stats.lognorm.fit(df.hs.values,) # shape, loc, scale
        pdf_lognorm = stats.lognorm.pdf(h1, param[0], loc=param[1], scale=param[2])
        pdf_Hs1 = pdf_lognorm
    
    param = Weibull_method_of_moment(df.hs.values) #stats.weibull_min.fit(df.hs.values) # shape, loc, scale
    pdf_Hs2 = stats.weibull_min.pdf(h1, param[0], loc=param[1], scale=param[2])
    
    
    # Find the index where two PDF cut, between P60 and P99 
    for i in range(len(h1)):
        if abs(h1[i]-np.percentile(df.hs.values,60)) < 0.1:
            i1=i
            
        if abs(h1[i]-np.percentile(df.hs.values,99)) < 0.1:
            i2=i
   ...
   # breakpoint()
    ...
    f_Hs_Tp = np.zeros((len(h1), len(t)))
    pdf_Hs_Tp=f_Hs_Tp*0
    
    for i in range(len(h1)):
        mu = a1 + a2*h1[i]**a3
        std2 = b1 + b2*np.exp(-b3*h1[i])
        std = np.sqrt(std2)
    ...
    for i in range(len(periods)):
        t3_val, h3_val, X_val, hs_tpl_tph_val = Hs_Tp_curve(df.hs.values, pdf_Hs, pdf_Hs_Tp, f_Hs_Tp, h1, t, interval, X=periods_adj[i])
        t3.append(t3_val)
        h3.append(h3_val)
        X.append(X_val)
    ...
    return a1, a2, a3, b1, b2, b3, pdf_Hs, h1, t3,h3,X,hs_tpl_tph 


def joint_distribution_W10_Hs_Tp(data,var_w10='w10',var_hs='hs',var_tp='tp',periods=[1,10,100,10000], adjustment=None):  
    import scipy.stats as stats
    from matplotlib import pyplot as plt
    from scipy.optimize import curve_fit
    from scipy.signal import find_peaks
    ...
    This fuction will plot Hs-Tp joint distribution using LogNoWe model (the Lognormal + Weibull distribution) 
    df : dataframe, 
    var1 : W10: mean wind speed (at 10m height)
    var2 : Hs: significant wave height,
    var3 : Tp: Peak period 
    file_out: W10-Hs-Tp joint distribution, optional
    """
    if adjustment == 'NORSOK':
        periods_adj = np.array([x * 6 for x in periods])
    else:
    ...
    period = np.array(periods)
    pd.options.mode.chained_assignment = None  # default='warn'
    df.loc[:,'w10'] = df[var_w10].values
    df.loc[:,'hs'] = df[var_hs].values
    df.loc[:,'tp'] = Tp_correction(df[var_tp].values)
    
   
    def marg_dist_mean_wind(W10, HS, TP):
        w1 = np.linspace(start=0.0, stop=np.ceil(max(W10)*np.sqrt(np.pi)), num=1520)#len(W10))
        pdf_W10 = w1*0
        param = Weibull_method_of_moment(W10) #stats.weibull_min.fit(df.hs.values) # shape, loc, scale
        pdf_W10 = stats.weibull_min.pdf(w1, param[0], loc=param[1], scale=param[2])
       # dff = pd.DataFrame({'W10' : W10,'pdf_W10': pdf_W10})#, 'HS' : HS,'TP' : TP})
        return pdf_W10#, dff

    #pdf_W10, dff_u = marg_dist_mean_wind(df[var_w10].values,df[var_hs].values,df[var_tp].values)
    pdf_W10 = marg_dist_mean_wind(df[var_w10].values,df[var_hs].values,df[var_tp].values)

   # breakpoint()
    def cond_param_HS_on_W10(HS,W10,TP,pdf_W10):
        h1 = np.linspace(start=0.01, stop=np.ceil(max(HS)*np.sqrt(np.pi)), num=1520)
        pdf_HS = h1*0
        param = Weibull_method_of_moment(HS) #stats.weibull_min.fit(df.hs.values) # shape, loc, scale
        pdf_HS = stats.weibull_min.pdf(h1, param[0], loc=param[1], scale=param[2])
      ...
      #  breakpoint()
        ...
        beta_bin = []
        intx = 1 
        w10_bin = np.arange(0,maxW10+intx,intx)
        for i in range(len(w10_bin)-1):
            idxs = np.where((w10_bin[i]<=W10) & (W10<w10_bin[i+1]))
         #   breakpoint() 
            if W10[idxs].shape[0] > 15 :  #to have  minimum set of number (15) to do the calculations on.
                param2 = Weibull_method_of_moment(HS[idxs]) #stats.weibull_min.fit(df.hs.values) # shape, loc, scale
                #pdf_Hs = stats.weibull_min.pdf(h1, param[0], loc=param2[1], scale=param2[2])
                alpha_bin.append(param2[0])
                beta_bin.append(param2[2])
                mean_w10.append(np.mean(W10[idxs]))
                mean_h1.append(np.mean(HS[idxs])) 

        mean_w10 = np.asarray(mean_w10)
        mean_h1 = np.asarray(mean_h1)

        alpha_bin = np.asarray(alpha_bin)
        beta_bin = np.asarray(beta_bin)

        w10 = np.linspace(start=0.0, stop=40, num=1520)

        # The power functions for alpha and beta
        def power_alpha(u, a1, a2,a3):
            return a1 + a2*(u**a3) 
 
        def power_beta(u, b1, b2, b3):
            return b1 + b2*(u**b3)
        
        # fit alpha
        a_params, a_cov = curve_fit(power_alpha, mean_w10, alpha_bin)#, maxfev=10000) 
        a1, a2, a3 = a_params
        param_h = Weibull_method_of_moment(HS) 
        #aa_params, aa_cov = curve_fit(power_alpha, W10, param_h[0])#, maxfev=10000) 
        #aa1, aa2, aa3 = aa_params

        # fit beta
        b_params, b_cov = curve_fit(power_beta, mean_w10, beta_bin)#, maxfev=10000) 
        b1, b2, b3 = b_params
        #bb_params, bb_cov = curve_fit(power_beta, W10, param_h[2])#, maxfev=10000) 
        #bb1, bb2, bb3 = bb_params
    
        f_Hs_W101 = np.zeros((len(h1),len(w10)))
        #f_Hs_W10 = np.zeros((len(h1),len(w10)))
        f_Hs_W10 = np.zeros((len(h1),(38)))

        pdf_Hs_W10=f_Hs_W10*0
        
       # breakpoint()
        ###
        dff_pand = pd.DataFrame()
        for i in range(len(w10_bin)-1):
        #for i in range(len(w10)):    
            idxs = np.where((w10_bin[i]<=W10) & (W10<w10_bin[i+1]))
            idxss = np.where((w10_bin[i]<=w10) & (w10<w10_bin[i+1]))
            #breakpoint() 
            if W10[idxs].shape[0] > 15 : 
                alpha1 = []
                beta1 = [] ; HS_ = []; W10_ = []; TP_ = []
                f_Hs_W102 = []
                alpha2 = a1 + a2*np.mean(w10[idxss])**a3
                beta2 = b1 + b2*np.mean(w10[idxss])**b3
                alpha = (a1 + a2*np.mean(W10[idxs])**a3)
                beta = (b1 + b2*np.mean(W10[idxs])**b3)
                alpha1.append(alpha)
                beta1.append(beta)
               # breakpoint()
                f_Hs_W101[i,:] = (alpha2/beta2)*(h1/beta2)**(alpha2-1)*np.exp(-(h1/beta2)**alpha2)
                f_Hs_W10[i,:] = (alpha2/beta2)*(h1[idxss]/beta2)**(alpha2-1)*np.exp(-(h1[idxss]/beta2)**alpha2)
                f_Hs_W102.append((alpha/beta)*(HS[idxs]/beta)**(alpha-1)*np.exp(-(HS[idxs]/beta)**alpha))
                #f_Hs_W102 = np.asarray(f_Hs_W102).squeeze()
               # breakpoint()
                HS_.append(np.mean(HS[idxs]))
                W10_.append(np.mean(W10[idxs]))
                TP_.append(np.mean(TP[idxs]))
               # breakpoint()
             #   plt.plot(np.mean(W10[idxs]),alpha2,'rd')
             #   plt.plot(np.mean(W10[idxs]),beta2,'bd')
             #   plt.plot(np.mean(W10[idxs]),alpha,'r*')
             #   plt.plot(np.mean(W10[idxs]),beta,'b*')
               # plt.plot(np.log(HS[idxs]),np.log(-np.log(1-f_Hs_W102)))
             #   flattened_list = [item for sublist in f_Hs_W102 for item in sublist]
             #   plt.plot(HS_[idxs],flattened_list,'*-')
                #plt.show()
        #breakpoint()
                dff = pd.DataFrame({'W10' : W10_,'HS': HS_, 'TP': TP_, 'f_Hs_W102': (f_Hs_W102)}) 
                #dff = pd.DataFrame({'W10' : W10_,'HS': HS_, 'TP': TP_, 'f_Hs_W102': np.mean(f_Hs_W102)}) 
                dff.dropna(inplace=True)
                dff_pand = pd.concat([dff_pand, dff])#, ignore_index=True)
        #plt.show()
        breakpoint()

        fig, ax = plt.subplots(figsize=(8,6))
        for i in range(len(w10)):
            alpha = a1 + a2*w10[i]**a3
          #  alpha2 = aa1 + aa2*w10[i]**aa3
            beta = b1 + b2*w10[i]**b3
          #  beta2 = bb1 + bb2*w10[i]**bb3

            f_Hs_W10[i,:] = (alpha/beta)*(h1/beta)**(alpha-1)*np.exp(-(h1/beta)**alpha)
            pdf_Hs_W10[i,:] = pdf_W10[i]*f_Hs_W10[i,:]
            
            #pdf_Hs_W10[i,:] = pdf_Hs[i]*f_Hs_Tp[i,:]
            plt.plot(w10[i],alpha,'r*')
            plt.plot(w10[i],beta,'b*')
          #  plt.plot(w10[i],alpha2,'rd')
          #  plt.plot(w10[i],beta2,'bd')
        plt.show()
       # breakpoint()
        return f_Hs_W10, pdf_Hs_W10, dff_pand
    
    f_Hs_W10, pdf_Hs_W10, dff_hs_u = cond_param_HS_on_W10(df[var_hs].values,df[var_w10].values,df[var_tp].values, pdf_W10)
    breakpoint()

    def cond_param_Tp_given_Hs_W10(TP,HS,W10):
        # The power functions for alpha and beta
        def power_fitting(x, y1, y2,y3):
            return y1 + y2*(x**y3) 
        
        ##
     #   param_h = Weibull_method_of_moment(HS) 
     #   aa_params, aa_cov = curve_fit(power_fitting, W10, param_h[0])#, maxfev=10000) 
     #   aa1, aa2, aa3 = aa_params
     #  # aa1 = 1.261; aa2 = 2.09e-09; aa3 = 0.159 #me
     #   #aa1 = 1.89; aa2 = 0.012; aa3 = 1.74 #from article
     #   bb_params, bb_cov = curve_fit(power_fitting, W10, param_h[2])#, maxfev=10000) 
     #   bb1, bb2, bb3 = bb_params
     #   bb1 = 1.908; bb2 = -1.98e-09; bb3 = 0.158
     #   #bb1 = 0.929; bb2 = 0.024; bb3 = 1.827 #from article
     #   ##
        cov = np.std(HS)/np.mean(HS) #this only gives one point to be sent into curve fitting, and that I feel is wrong
        
        eee_params, eee_cov = curve_fit(power_fitting, HS, TP, maxfev=10000)
        eee1, eee2, eee3 = eee_params
        fff_params, fff_cov = curve_fit(power_fitting, HS, W10, maxfev=10000)
        fff1, fff2, fff3 = fff_params
        g_params, g_cov = curve_fit(power_fitting, HS, cov, maxfev=10000) 
        g1, g2, g3 = g_params
        #g1,g2,g3 = 0.012,0.026,-0.887
        intw = 1.0
        inth = 0.5
        maxW10 = max(W10)
        maxHS = max(HS)
        w10_bin = np.arange(0,maxW10+intw,intw)
        HS_bin = np.arange(0.1,maxHS+inth,inth)
      #  breakpoint()
      #  h1 = np.linspace(start=0.01, stop=np.ceil(max(HS)*np.sqrt(np.pi)), num=1500)
        #f_Tp = np.zeros((len(w10_bin), len(HS_bin))) # f_Tp = [[] for _ in range(len(w10_bin))]
        #f_Hs = np.zeros((len(w10_bin), len(HS_bin)))
        #breakpoint()
        #df = []
        h1 = np.linspace(start=0.1, stop=np.ceil(max(HS)*np.sqrt(np.pi)), num=1500)
        w10 = np.linspace(start=0.01, stop=30, num=1500)
        f_Hs_W10 = np.zeros((len(h1),len(w10_bin)-1))

        dff_panda = pd.DataFrame()
        for iw in range(len(w10_bin)-1):
            idxs = np.where((w10_bin[iw]<=W10) & (W10<w10_bin[iw+1]))

            if W10[idxs].shape[0] > 100 :  #to have  minimum set of number (15) to do the calculations on.
               # breakpoint()
                
            #    alpha = aa1 + aa2*np.mean(W10[idxs])**aa3
            #    beta = bb1 + bb2*np.mean(W10[idxs])**bb3
            #    plt.plot(np.mean(W10[idxs]),alpha,'rd')
            #    plt.plot(np.mean(W10[idxs]),beta,'bd')
            #    plt.plot(np.mean(W10[idxs]),np.mean(HS[idxs]),'g*')
                #plt.show()
                #f_Hs_W10 = []
                #breakpoint()
                #f_Hs_W10[iw,:] = (alpha/beta)*(h1/beta)**(alpha-1)*np.exp(-(h1/beta)**alpha)
                #f_Hs_W10.append((alpha/beta)*(h1/beta)**(alpha-1)*np.exp(-(h1/beta)**alpha))
            #    f_Hs_W10.append((alpha/beta)*(HS[idxs]/beta)**(alpha-1)*np.exp(-(HS[idxs]/beta)**alpha))

                #####
                mean_w10_ = [] ; mean_w10 = [] 
                mean_hs_ = [] ; mean_hs = [] 
                mean_tp_ = [] ; mean_tp = [] 
                W10_bin_ = [] ; HS_bin_ = []; hs_data = []
                t_bar22 = []; u_bar22 = []; COV = []
                tp_ = []; std_tp = []; std_hs = []
                ...
                HS_ = HS[idxs]
                TP_ = TP[idxs]
                #calc for each bin in Hs: 
                for ih in range(len(HS_bin)-1):
                    if (HS[idxs].shape[0] > 100):
                        idhs = np.where((HS_bin[ih]<=HS_) & (HS_<HS_bin[ih+1])) 
                        if not math.isnan(np.mean(HS_[idhs])):
                          #  breakpoint()
                            mean_w10_.append(np.mean(W10_[idhs]))
                            mean_hs_.append(np.mean(HS_[idhs])) 
                            mean_tp_.append(np.mean(TP_[idhs])) 
                            tp_.append(np.mean(TP_[idhs]))
                            W10_bin_.append(w10_bin[iw])
                            HS_bin_.append(HS_bin[ih])
                            hs_data.append(HS_[idhs])
                            std_hs.append(np.std(HS_[idhs]))
                            std_tp.append(np.std(TP_[idhs])) 
                            t_b22 = eee1 + eee2*np.mean(HS_[idhs])**eee3
                            u_b22 = fff1 + fff2*np.mean(HS_[idhs])**fff3
                            cov_ = g1 + g2*np.mean(HS_[idhs])**g3
                            t_bar22.append(t_b22)
                            u_bar22.append(u_b22)
                            COV.append(cov_)
                           # plt.plot(np.mean(HS_[idhs]),cov_,'*')
                        
                
                #mean_w10 = np.asarray(mean_w10_)[~np.isnan(np.asarray(mean_w10_))]   
                #mean_hs = mean_hs_# np.asarray(mean_hs_)[~np.isnan(np.asarray(mean_hs_))]
                #mean_tp = np.asarray(mean_tp_)[~np.isnan(np.asarray(mean_tp_))]
                dff = pd.DataFrame({ 'W10_bin': W10_bin_, 'Mean_W10': mean_w10_, 'HS_bin': HS_bin_, 'Mean_HS': mean_hs_, 'Mean_TP': mean_tp_, 'Mean_TP': mean_tp_}) 
                dff['std_tp'] =  std_tp
                dff['tp_'] =  tp_
                dff['u_bar22'] =  u_bar22
                dff['t_bar22'] = t_bar22 
                dff['COV'] = COV
              #  dff['f_Hs_W10'] = f_Hs_W10
                dff.dropna(inplace=True)
                dff_panda = pd.concat([dff_panda, dff])#, ignore_index=True)

               # breakpoint()

                # fit t_bar(h) and u_bar(h), eq.(18+19)
            #     e_params, e_cov = curve_fit(power_fitting, mean_hs_, mean_tp_, maxfev=10000) #cannot send in nan's
            #     e1, e2, e3 = e_params
            #     f_params, f_cov = curve_fit(power_fitting, mean_hs_, mean_w10_, maxfev=10000) 
            #     f1, f2, f3 = f_params

            #     dff['ef_Hs_W10, pdf_Hs_W101'] = e1; dff['e2'] = e2; dff['e3'] = e3
            #     dff['f1'] = f1; dff['f2'] = f2; dff['f3'] = f3
            #     #breakpoint()
            #     ##test2 with HS,Tp,W10 and not mean                
            #     ee_params, e_cov = curve_fit(power_fitting, HS_, TP_, maxfev=10000)
            #    # ee1_params, e1_cov = curve_fit('lm', HS_, TP_,maxfev=10000)
            #     ee1, ee2, ee3 = ee_params
            #     ff_params, e_cov = curve_fit(power_fitting, HS_, W10_, maxfev=10000)
            #     ff1, ff2, ff3 = ff_params
            #     dff['ee1'] = ee1; dff['ee2'] = ee2; dff['ee3'] = ee3
            #     dff['ff1'] = ff1; dff['ff2'] = ff2; dff['ff3'] = ff3

            #     t_bar = []; t_bar2 = []; t_bar22 = []
            #     u_bar = []; u_bar2 = []; u_bar22 = []
            #     hlin = np.linspace(0.2,max(mean_hs_),len(mean_hs_))
            #     for ij in range(len(mean_hs_)):
            #         t_b = e1 + e2*mean_hs_[ij]**e3
            #         t_b2 = ee1 + ee2*hlin[ij]**ee3
            #         t_b22 = eee1 + eee2*hlin[ij]**eee3
            #         u_b = f1 + f2*mean_hs_[ij]**f3
            #         u_b2 = ff1 + ff2*hlin[ij]**ff3
            #         u_b22 = fff1 + fff2*hlin[ij]**fff3
            #         t_bar.append(t_b)
            #         t_bar2.append(t_b2)
            #         t_bar22.append(t_b22)
            #         u_bar.append(u_b)
            ...
            #         u_bar22.append(u_b22)
                    #breakpoint()
             #   dff['t_bar'] = t_bar 
             #   dff['t_bar2'] = t_bar2 dff['t_bar22'] = t_bar22 
             #   dff['u_bar'] =  u_bar
             #   dff['u_bar2'] =  u_bar2
             ...
             #   dff['f_t_u_h'] =  f_t_u_h
              ...
              #  breakpoint()
             #   plt.plot(HS_,W10_,'-')
             #   plt.plot(dff.Mean_HS,dff.u_bar,'-*')
             #   plt.plot(dff.Mean_HS,dff.u_bar2,'-o')
             #   plt.plot(dff.Mean_HS,dff.u_bar22,'-d')
    ...
    #f_Hs_W10, pdf_Hs_W10 = cond_param_Tp_given_Hs_W10(df[var_tp].values,df[var_hs].values,df[var_w10].values)
    dff_p = cond_param_Tp_given_Hs_W10(df[var_tp].values,df[var_hs].values,df[var_w10].values)
   # breakpoint()
    ...
    def remove_outliers(data):
        ...
        data_out = data[~((data < (Q1)) | (data>Q3))]
        return data_out


    def finding_gamma(dff_p):
        ind = (dff_p.HS_bin==2.1)
        norm_period = ((dff_p.Mean_TP[ind] - dff_p.t_bar22[ind]) / dff_p.t_bar22[ind])
        norm_wind = (dff_p.Mean_W10[ind] - dff_p.u_bar22[ind])/dff_p.u_bar22[ind]
      #  plt.plot(norm_wind,norm_period,'r*')
      #  plt.show()
        gamma = 1
        return gamma
    
    gamma = finding_gamma(dff_p)

    def finding_theta(dff_p):
        harange  = np.arange(0.1,dff_p.HS_bin.max()+0.5,0.5)
        theta_ = []
        for i in range(len(harange)-1):
            ind = (dff_p.HS_bin==harange[i])
            norm_period = ((dff_p.Mean_TP[ind] - dff_p.t_bar22[ind]) / dff_p.t_bar22[ind])
            norm_wind = (dff_p.Mean_W10[ind] - dff_p.u_bar22[ind])/dff_p.u_bar22[ind]
            the = (norm_period/norm_wind)
            the_ = np.mean(the)
            theta_.append(the_)
            #plt.plot(harange[i],the_,'r*')
            #plt.plot(norm_wind,norm_period,'b*')
        theta2_ = remove_outliers(theta_)
        theta = theta2_.mean()
        #theta2 = np.asarray(theta2_[1:-4]).mean() #this is universal and need to be changed
        plt.plot(theta2_,'*') #visual sanity check 
        plt.show()
        #breakpoint()
        return theta
    
    theta = finding_theta(dff_p)

    def calc_eq_15(dff_p,gamma,theta):
      #  gamma = gamma #1
       # theta = theta #-0.489
        mu_tp = np.array(dff_p.t_bar22) * [1 + theta*((np.array(dff_p.Mean_W10)-np.array(dff_p.u_bar22))/np.array(dff_p.u_bar22))**gamma]
        mu_tp = mu_tp.squeeze()
        sigma_tp = dff_p.std_tp
      #  breakpoint()
        nu_tp = sigma_tp/mu_tp
        sigma_lntp = np.sqrt(np.log(nu_tp**2 + 1))
        mu_lntp = np.log(mu_tp/np.sqrt(1+nu_tp**2))

        f_t_u_h = (1/(np.sqrt(2*np.pi)*sigma_lntp)*np.asarray(dff_p.tp_))*np.exp(-0.5*((np.log(dff_p.tp_)-mu_lntp)/sigma_lntp)**2)
        dff_p['f_t_u_h'] = f_t_u_h
      #  breakpoint()
        return f_t_u_h, dff_p
    

    f_t_u_h, dff_tp_u_hs = calc_eq_15(dff_p,gamma,theta)


    breakpoint()
# ##this one: is it correct to take in Mean_TP? 
#     #harange = np.arange(dff_p.Mean_HS.min(),dff_p.Mean_HS.max(),0.5)
#     harange  = np.arange(0.1,10.6+0.5,0.5)
#     theta = []; theta2 = []
#     for i in range(len(harange)-1):
#         ind = (dff_p.HS_bin==2.1)# & (dff_p.W10_bin==7.5)#harange[i]) #get h_bin
#       #  ind = (dff_p.HS_bin==harange[i])# & (dff_p.W10_bin==7.5)#harange[i]) #get h_bin
#         norm_period = ((dff_p.Mean_TP[ind] - dff_p.t_bar22[ind]) / dff_p.t_bar22[ind])
#         norm_wind = (dff_p.Mean_W10[ind] - dff_p.u_bar22[ind])/dff_p.u_bar22[ind]
#         the = (norm_period/norm_wind)
#       #  breakpoint()
#        # thet = [element for element in the if abs(element) < 0.5]
#         #theta_ = (norm_period/norm_wind).mean()
#         theta_ = np.mean(the)
#         plt.plot(norm_wind,norm_period,'r*')
#         #plt.plot(harange[i],theta_,'r*')
#         theta.append(theta_)
#       #  breakpoint()
#     ##test2:
#     #     ind2 = ind#((dff_p.hlin<=harange[i+1]) & (dff_p.hlin>harange[i])) #get h_bin 
#     #     #breakpoint()
#     #     ww_ind = np.linspace(dff_p.Mean_W10[ind2].min(),dff_p.Mean_W10[ind2].max(),(dff_p.Mean_W10[ind2].shape[0]))
#     #     norm_period2 = ((dff_p.Mean_TP[ind2] - dff_p.t_bar2[ind2]) / dff_p.t_bar2[ind2])
#     #     norm_wind2 = ((dff_p.Mean_W10[ind2] - dff_p.u_bar2[ind2])/dff_p.u_bar2[ind2])  # MÅ SENDE INN LINSPACE AV WIND
#     #     #norm_wind2 = ((dff_p.W10_bin[ind2] - dff_p.u_bar2[ind2])/dff_p.u_bar2[ind2])  # MÅ SENDE INN LINSPACE AV WIND
#     #     #norm_wind2 = ((ww_ind - dff_p.u_bar2[ind2])/dff_p.u_bar2[ind2])  # MÅ SENDE INN LINSPACE AV WIND
#     #     the2_ = (norm_period2/norm_wind2)
#     #   #  thet2 = [element for element in the2_ if abs(element) < 0.5]
#     #     theta2_ = np.mean(the2_)
#     #     #plt.plot(norm_wind2,norm_period2,'b*')
#     #     plt.plot(harange[i],theta2_,'b*')
#     #     theta2.append(theta2_)
#       # breakpoint()
#     plt.show()  
#     theta_mean = np.nanmean(theta)
#    # theta2_mean = np.nanmean(theta2)  
#     theta2 = np.asarray(theta[1:-4]).mean()
#     breakpoint()



    return #pdf_W10, f_Hs_W10, pdf_Hs_W10
